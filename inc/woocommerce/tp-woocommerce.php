...
if (!function_exists('ampere_content_single_details')) {
    function ampere_content_single_details()
    {
        global $product;
        global $post;
        global $woocommerce;
        ...
        <div class="tp-shop-details__right-warp">
            ...
            <?php if (!empty($rating)) : ?>
                <div class="tp-shop-details__ratting d-flex">
                    ...
                    <span class="review-text">( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'ampere');
            ...
            <?php else : ?>
                <div class="tp-shop-details__ratting">
                    ...
                    <span><i class="fal fa-star"></i></span>
                    <span><i class="fal fa-star"></i></span>
                    <span>( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'ampere');
                                                                    echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
                </div>
            <?php endif; ?>
...
add_action('woocommerce_single_product_summary', 'ampere_content_single_details', 4);
 ...
 *************************************************/
function ampere_shipping_progress_bar()
{
...
if (get_theme_mod('shipping_progress_bar_location_card_page', 0) == '1') {
    add_action('woocommerce_before_cart_table',  'ampere_shipping_progress_bar');
}

if (get_theme_mod('shipping_progress_bar_location_mini_cart', 0) == '1') {
    add_action('woocommerce_before_mini_cart_contents', 'ampere_shipping_progress_bar');
}

if (get_theme_mod('shipping_progress_bar_location_checkout', 0) == '1') {
    add_action('woocommerce_checkout_before_customer_details', 'ampere_shipping_progress_bar');
}
...
function ampere_sale_percentage()
{
    global $product;
    $output = '';
...
if (!function_exists('ampere_header_add_to_cart_fragment')) {
    function ampere_header_add_to_cart_fragment($fragments)
    {
        ob_start();
    ?>
...
add_filter('woocommerce_add_to_cart_fragments', 'ampere_header_add_to_cart_fragment');


// product-content archive
if (!function_exists('ampere_content_product_grid')) {
    function ampere_content_product_grid()
    {
        global $product;
        global $post;
        global $woocommerce;
                        ...
                        <div class="product__action d-flex flex-column flex-wrap">
                            ...
                            <?php if (function_exists('woosw_init')) : ?>
                                <div class="product-action-btn product-add-wishlist-btn p-relative">
                                    ...
                                    <span class="product-action-tooltip"><?php echo esc_html__('Add To Wishlist', 'ampere'); ?></span>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (class_exists('WPCleverWoosq')) : ?>
                                <div class="product-action-btn p-relative">
                                    ...
                                    <span class="product-action-tooltip"><?php echo esc_html__('Quick view', 'ampere'); ?></span>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (function_exists('woosc_init')) : ?>
                                <div class="product-action-btn p-relative">
                                    ...
                                    <span class="product-action-tooltip"> <?php echo esc_html__('Add To Compare', 'ampere'); ?></span>
                                </div>
                            <?php endif; ?>
...
add_action('woocommerce_before_shop_loop_item', 'ampere_content_product_grid', 10);


// smart quickview
add_filter('woosq_button_html', 'ampere_woosq_button_html', 10, 2);
function ampere_woosq_button_html($output, $prodid)
{
...
function woocommerce_template_loop_add_to_cart($args = array())
{
    ...
    if ($product->is_type('simple')) {
        $btntext = esc_html__("Add to Cart", 'ampere');
    } elseif ($product->is_type('variable')) {
        $btntext = esc_html__("Select Options", 'ampere');
    } elseif ($product->is_type('external')) {
        $btntext = esc_html__("Buy Now", 'ampere');
    } elseif ($product->is_type('grouped')) {
        $btntext = esc_html__("View Products", 'ampere');
    } else {
        $btntext = "Add to Cart";
    }
        ...
        $args = wp_parse_args(
            ...
            apply_filters(
                ...
                array(
                    ...
                    'before'      => '',
                    'after'       => '',
                    'home'        => _x('Home', 'breadcrumb', 'ampere'),
                )
            )
        );
...
/*************************************************
## ampere Woo Search Form
 *************************************************/

add_filter('get_product_search_form', 'ampere_custom_product_searchform');

function ampere_custom_product_searchform($form)
{

    $form = '<div class="search-form p-relative">
                <form action="' . esc_url(home_url('/shop')) . '" role="search" method="get" id="searchform">
                <input type="search" value="' . get_search_query() . '" name="s" placeholder="' . esc_attr__('Search Product', 'ampere') . '" autocomplete="off">
